...
import it.bls.mybatistest.service.BlogService;
...
public class BlogServiceTest {
	...
	public void testInsertBlog() {
		Blog blog = new Blog();
		blog.setBlogTitle("test_blog_" + System.currentTimeMillis());
		...
		Assert.assertEquals(blog.getBlogTitle(), createdBlog.getBlogTitle());
	}
	...
	public void testUpdateBlog() {
		...
		blog.setBlogTitle("TestBlogName" + timestamp);
		blogService.updateBlog(blog);
		Blog updatedBlog = blogService.getBlogById(2);
		Assert.assertEquals(blog.getBlogTitle(), updatedBlog.getBlogTitle());
	}
